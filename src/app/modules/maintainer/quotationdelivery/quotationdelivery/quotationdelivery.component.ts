...
@Component({
  ...
  styleUrls: ['./quotationdelivery.component.css']
})
export class QuotationdeliveryComponent implements OnInit, OnDestroy {

  public subscriptions: Array<Subscription> = [];
  public dataTable: DataTableModel;
  public elementListData: Array<QuotationDeliveryModel> = [];
  dtOptions: DataTables.Settings = {};
  public tblData: QuotationDeliveryModel[] = [];
  ...
  constructor(
    ...
    private elementService: QuotationdeliveryService
  ) { }
  ...
  redirectToEdit(element: QuotationDeliveryModel) {
    this.elementService.elementSelected = element;
    this.router.navigate(['/maintainer/add-upd-quotationdelivery']);
  }

  redirectToView(element: QuotationDeliveryModel) {
    this.elementService.elementSelected = element;
    this.router.navigate(['/maintainer/add-upd-quotationdelivery/ver']);
  }


  async onDelete(elementSelected: QuotationDeliveryModel) {

    const resultModal = await this.modalService.open(
      {
        titulo: 'Eliminar el registro de entrega',
        texto: `¿Esta seguro que desea eliminar el registro de entrega "${elementSelected.description}"?`,
        icono: 'warning',
    ...
    if (resultModal) {
      ...
      this.subscriptions.push(this.elementService.delete(elementSelected).subscribe(async result => {
        ...
        const resultModal = await this.modalService.open(
          {
            titulo: 'Eliminado',
            texto: `El registro de entrega "${elementSelected.description}" se ha eliminado correctamente.`,
            icono: 'success',
